...
function AliwalController( pAliwalModel ){
	...
	// Yahoo Events
	this.eventGeocodeSuccess = new YAHOO.util.CustomEvent("AliwalControllerGeocodeSuccess", this);
	this.eventGeocodeFail    = new YAHOO.util.CustomEvent("AliwalControllerGeocodeFail", this);
	
	// Privileged method
	this.addPlacemark = function( pAliwalPlacemark ){
		_model.addPlacemark( pAliwalPlacemark );
	}
	
	// Privileged method
	this._geocodeFromCache = function( pAliwalPlacemark ){
		/**
...
// Public members
AliwalController.prototype.geocodePlacemark = function( pPlacemark, pCallback ){
	/** 
	...
	// Try the cache 1st
	try{
		this._geocodeFromCache( pPlacemark);
	} catch(e){
		// Ignore Dummied out cache or cache miss exceptions
	}
	
	if( pPlacemark.isGeocoded() ){
		this.eventGeocodeSuccess.fire();
		pCallback( pPlacemark );
	} else {
...
AliwalController.prototype.loadDelimitedFile = function(pFile,pDelimiters,
 ...
 * Asyncronous, calls pCallback when done.
	...
	try {
		...
		while(hasmore){
			...
			if( placemark.isGeocoded() ){
				...
				this.geocodePlacemark(placemark, this.addPlacemark );
			};
		};
...
AliwalController.prototype.loadKMLFile = function(	pFile, 
													pLoadHandler, 
													pProgressHandler, 
													pErrorHandler, 
													pCallback ){
	    	...
	    	if(geocoder.status == 200 || geocoder.status == 0){
				...
				$(xmlmarks).find('Placemark').each( function(key, xmark){
					...
					if(doGeocode){
						...
						// ToDo: Geocoding failures ???
						that.geocodePlacemark(placemark, that.addPlacemark );
					};
				});
				pCallback();
			} else {
...
AliwalController.prototype.loadDefaultData = function( pCallback ){
	/** 
	 * Load some dummy data into the model
	 * Asyncronous, calls pCallback when done (to match the load*File() methods ).
	 */
	var that = this;
	var fDelayed = function(pCallback){
		...
		that.addPlacemark(x20);

		pCallback();
	};
	window.setTimeout(fDelayed, 111, pCallback)
} 

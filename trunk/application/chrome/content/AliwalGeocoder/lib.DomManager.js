...
function DomManager(){
	...
	this.drawLabelSelector = function( pDivId ){
		...
		$.each( that.pinLabels, function(i,val){
			sel += '<OPTION>'+val+'</OPTION>\n';
		});
		sel += '</SELECT>';
		if( that.pinLabels.length > 0){
			dd.append( sel );
		} else {
	...
	this.drawPopupSelector= function( pDivId ){
		/* Args: The string id of the div to draw the selector in 
		 * Privileged method.
		 */
		var dd = $('#' + pDivId);
		dd.empty();
	...
	this.getPopupSelection = function( ){
		/* returns an array of members that should be shown in popups.
		 * Used to get this from the GUI, now just returns all the columns
		 * Privileged method.
		 */
		// New version
	...
	this.drawControls = function( pPoll){
		/* Draws the GUI controls.
		 * Privileged Method
		 */
		if( pPoll){
			if(xscopeNS.flags.loadingData){
				setTimeout( domMgr.drawControls,555, true );
			} else {
				setTimeout( domMgr.drawControls,1, false );
			}
		} else {
			// Filtering stuff
			...
			for (var pi in pinItems){
				that.pinLabels.push(pi);
			}
			domMgr.drawLabelSelector('pin_label_selector' );
